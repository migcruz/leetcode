/*
 * LeetCode Problem 2042: Check if Numbers Are Ascending in a Sentence
 * Difficulty: Easy
 *
 * Problem Description:
 * A sentence is a list of tokens separated by a single space with no leading or trailing spaces.
 * Every token is either a positive number consisting of digits 0-9 with no leading zeros,
 * or a word consisting of lowercase English letters.
 *
 * Given a string s representing a sentence, check if all the numbers in s
 * are strictly increasing from left to right.
 *
 * Return true if so, or false otherwise.
 *
 * Examples:
 *
 * Example 1:
 * Input:  s = "1 box has 3 blue 4 red 6 green and 12 yellow marbles"
 * Output: true
 * Explanation: The numbers in s are: 1, 3, 4, 6, 12 — all strictly increasing.
 *
 * Example 2:
 * Input:  s = "hello world 5 x 5"
 * Output: false
 * Explanation: The numbers in s are: 5, 5 — not strictly increasing.
 *
 * Example 3:
 * Input:  s = "sunset is at 7 51 pm overnight lows will be in the low 50 and 60 s"
 * Output: false
 * Explanation: The numbers in s are: 7, 51, 50, 60 — not strictly increasing.
 *
 * Constraints:
 *   3 <= s.length <= 200
 *   s consists of lowercase English letters, spaces, and digits from 0 to 9.
 *   The number of tokens in s is between 2 and 100, inclusive.
 *   Tokens are separated by a single space.
 *   There are at least two numbers in s.
 *   Each number is a positive integer less than 100 with no leading zeros.
 *   s contains no leading or trailing spaces.
 */

bool areNumbersAscending(char* s) {

    // we need to extract every number in the sentence and make sure the numbers are in increasing order

    char *token = strtok(s, " ");
    int prevNum = 0;
    int currNum = 0;

    while (token != NULL) {
        // printf("%s\n", token);
        char *endPtr = NULL;
        int tempNum = strtol(token, &endPtr, 10); //base 10
        if (*endPtr == '\0') { // valid number
            currNum = tempNum;
            if (currNum <= prevNum) {
                return false;
            }
        }

        prevNum = currNum;

        token = strtok(NULL, " ");
    }
    
    return true;
}